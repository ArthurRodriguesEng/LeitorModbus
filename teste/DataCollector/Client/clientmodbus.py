...
class ClientMODBUS():
    ...
    def read_and_decode_data(self, EQP_number):
        print(self.tag +str(self._ID)+ " reading...")
        print(self.MB_Table)
        data_read = []
    ...
    def build_csv(self,log):
        df = pd.DataFrame(log)
        df.to_csv(str(self.tag) + '_'+ str(self._ID) + ".csv", header= False)

    def send(self):return
    
    def data_to_json(self,log, indent ):
        return json.dumps(log, indent = indent)

    
    def build_jsonfile(self, log):
        data_log = self.data_to_json(log,4)
        with open(str(self.tag) + '_'+ str(self._ID) +".json", "w") as outfile:
    ...
    def data_manager(self):
        ...
        if(self.log_to_send == []):
            file = str(self.tag+ str(self._ID) + ".csv")
