...
class TCUNCUClient(ClientMODBUS):
    ...
    def read_NCU_TCU(self):
        ...
        if(self._client.open() == True):
            ...
            self.log_to_send.append(log)
            
            i=0
            while i < len(self.log_to_send):
                if(self.insert_to_db(self.log_to_send[i],'RB_NCU')):
                    self.log_to_send.pop(i)
                else:
                    while i < len(self.log_to_send):
                        self.build_jsonfile(log, 'NCU', len(self.log_to_send))
                        self.build_csv(self.log_to_send, modbus_table, 'NCU'+ str(self._ID))
                        i += 1
                    break
