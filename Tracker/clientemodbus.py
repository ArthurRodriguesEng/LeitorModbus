...
class ClienteMODBUS():
    ...
    def __init__(self, server_ip, porta, usina_name , ID):
        try:
            ...
            try:
                ...
                if(int(modbus_table["n_bit"][tag_count]) == 32):
                    if(modbus_table["type"][tag_count] == "F32"):
                        decoder = self.read_data(modbus_table["func"][tag_count],int(modbus_table["address"][tag_count] + modbus_table["up_addr"][tag_count]*(EQP_number) +1), int(2))
                        decoder = self.decode_float32(decoder)
                    elif(modbus_table["type"][tag_count] == "U32"):
                        decoder = self.read_data(modbus_table["func"][tag_count],int(modbus_table["address"][tag_count] + modbus_table["up_addr"][tag_count]*(EQP_number)), int(2))
                        decoder = str(self.float_to_date(decoder))
                    else:
                        decoder = self.read_data(modbus_table["func"][tag_count],int(modbus_table["address"][tag_count] + modbus_table["up_addr"][tag_count]*(EQP_number) +1 ), int(2))
                        
                if(int(modbus_table["n_bit"][tag_count]) == 16):
                    decoder = self.read_data(modbus_table["func"][tag_count],int(modbus_table["address"][tag_count]+ modbus_table["up_addr"][tag_count]*(EQP_number)), int(1))
    ...
    def read_data(self, tipo, addr, n):
        ...
        if tipo == 1:
            result = self._cliente.read_coils(addr, n_bits = n)
            return result
        if tipo == 2:
            result = self._cliente.read_discrete_inputs(addr, n_bits = n)
            return result
        if tipo == 5:
            result = self._cliente.read_input_registers(addr, n_reg = n)
            return result
        if tipo == 6:
            result = self._cliente.read_holding_registers(addr, n_reg = n)
            return result
